...
Открывает дизассемблирование бинарника в соседней панели и связывает строки
исходника с инструкциями ассемблера цветовым маппингом — как в Compiler Explorer (Godbolt),
но для локальных бинарников.
...
После `Show Assembly` в статус-баре появляется информация:

```
$(file-binary) main | 12 funcs | gnu
```

- Имя бинарника, количество функций, тип objdump
- Клик по нему — быстрый refresh
